# app_comb_allROI_zscore_gs.py
# 画像特徴 → 縮瞳モデル（1段目, RF or XGB, GroupKFold + GridSearch）
# 加工 + *_orig → 画像特徴（2段目, RF or XGB, GridSearch）
# 1段目の「重要度 × 符号付き方向」で z を定義し、
# 2段目でその z を最大化する加工を探すツール
#
# 特徴量グループ:
#   ・all       : all_* 系
#   ・all_area  : all_area_* 系
#   ・all_pupil : all_pupil_* 系
#   ・ROI       : center_ / parafovea_ / periphery_ 系
#
# NEW:
#   - 1段目/2段目 GridSearch ON/OFF 切り替え
#   - 1段目: 「全部入りモデル」と「上位k特徴のみモデル」のCV性能を両方表示
#   - 2段目: CV当てはまりを表＋グラフで表示し、
#            その手前で処理を止めるオプション（逆推定スキップ）を追加
#   - 18パターン × ランダムサーチの結果に対して
#       * Q = SSIM による画質制御を導入
#       * ① 制約付き最適化（SSIM ≥ 閾値の中で z 最大）
#       * ② 合成指標 J = α·z − β·(1−SSIM) 最大化
#     のどちらかを選択可能に
...
# ==========================================
# 画質指標: SSIM (Structural Similarity)
# ==========================================
def compute_ssim_pil(img_ref: Image.Image, img_proc: Image.Image) -> float:
    """
    参照画像 img_ref と 加工後画像 img_proc の SSIM を計算する。
    どちらも PIL.Image (RGB) を想定。
    Wang et al. (2004) に準拠した簡易版。
    """
    # RGB -> Gray, 0-1 float
    ref = np.array(img_ref.convert("RGB")).astype("float32") / 255.0
    proc = np.array(img_proc.convert("RGB")).astype("float32") / 255.0
    ...
    mu1 = cv2.GaussianBlur(ref_gray, kernel_size, sigma)
    mu2 = cv2.GaussianBlur(proc_gray, kernel_size, sigma)

    mu1_sq = mu1 * mu1
    mu2_sq = mu2 * mu2
    mu1_mu2 = mu1 * mu2

    sigma1_sq = cv2.GaussianBlur(ref_gray * ref_gray, kernel_size, sigma) - mu1_sq
    sigma2_sq = cv2.GaussianBlur(proc_gray * proc_gray, kernel_size, sigma) - mu2_sq
    sigma12 = cv2.GaussianBlur(ref_gray * proc_gray, kernel_size, sigma) - mu1_mu2
...
def make_weighted_globals_for_single(roi_feats: dict):
    """
    roi_feats: {"center_bnL_mean": v, "parafovea_bnL_mean": v, ...}
    から
      - "all_area_<feat_name>"
      - "all_pupil_<feat_name>"
    を計算して dict で返す
    """
...
def main():
    ...
    tab1, tab2 = st.tabs(["📊 データ概要", "🧬 縮瞳に効く加工推薦（all/all_area/all_pupil/ROI, z最適化）"])
    ...
    # ===========================
    with tab2:
        st.header("🧬 2段モデルでの縮瞳向き加工推薦（all / all_area / all_pupil / ROI, z最適化）")
        ...
        # ★ ここで逆推定を最後までやるかどうか選べるようにした ★
        do_inverse = st.checkbox(
            "逆推定（ベスト加工探索）まで実行する",
            value=True,
            help="OFFにすると、2段目モデルの当てはまり（R²）を表示したところで処理を止めます。"
        )

        # === 画質制御パラメータ (SSIM) ===
        st.markdown("### 🎛 画質制御（SSIM を用いた品質管理）")

        optimization_mode = st.radio(
            "画質と z の扱い方",
            [
                "制約付き最適化（SSIM ≥ 閾値の中で z を最大化）",
                "合成指標 J = α·z − β·(1−SSIM) を最大化",
            ],
            index=0,
            help="どちらを選んでも、上位 N 個の候補について SSIM を評価し、その中からベスト加工を選びます。",
        )

        ssim_threshold = st.slider(
            "SSIM の下限値（制約付きモードで使用）",
            min_value=0.5,
            max_value=1.0,
            value=0.7,
            step=0.01,
        ...
        max_candidates_for_quality = st.slider(
            "SSIM を計算する候補数（z が大きい順に評価）",
            min_value=100,
            max_value=5000,
            value=1000,
            step=100,
            help="大きくすると精緻になりますが、そのぶん処理時間が増えます。",
        )

        alpha_J = st.number_input(
            "J = α·z − β·(1−SSIM) の α（z の重み）",
            value=1.0,
            step=0.1,
        )
        beta_J = st.number_input(
            "J = α·z − β·(1−SSIM) の β（画質ペナルティの重み）",
            value=1.0,
            step=0.1,
        )
        ...
        if st.button("🚀 モデル学習 & 推薦加工探索"):
            ...
            with st.spinner("1段目: 画像特徴 → 縮瞳 モデルを学習中 ..."):
                ...
                st.subheader("候補特徴量の重要度（縮瞳モデル・全部入り）")
                st.dataframe(imp_df_all.head(30), use_container_width=True)

                st.subheader("1段目 CV 結果（全部入りモデル, GroupKFold）")
                st.write(f"mean_train R²: **{cv_summary1_full['mean_train']:.3f}** ± {cv_summary1_full['std_train']:.3f}")
                st.write(f"mean_test  R²: **{cv_summary1_full['mean_test']:.3f}** ± {cv_summary1_full['std_test']:.3f}")
                if use_grid1:
                    st.write("ベストパラメータ（全部入り）:", best_params1)
                else:
                    st.write("ベストパラメータ: GridSearch OFF（デフォルト設定）")

                # 上位kだけ使用
                selected_features = imp_df_all["feature"].head(top_k).tolist()
                st.markdown("**このうち上位 k 個だけを使って、z を定義 & モデルを作り直します。**")
                ...
                st.subheader(f"1段目 CV 結果（上位 {top_k} 特徴のみモデル）")
                st.write(f"mean_train R²: **{cv_summary1_sel['mean_train']:.3f}** ± {cv_summary1_sel['std_train']:.3f}")
                st.write(f"mean_test  R²: **{cv_summary1_sel['mean_test']:.3f}** ± {cv_summary1_sel['std_test']:.3f}")

                # z 用の重み（上位kモデルから）
                imp_sel = rf_pupil.feature_importances_
                imp_df_sel = (
                    pd.DataFrame({"feature": selected_features, "importance": imp_sel})
                    .sort_values("importance", ascending=False)
                    .reset_index(drop=True)
                )
                ...
                st.subheader("zの重み（符号付き・正規化済, 上位kモデル）")
                st.dataframe(
                    pd.DataFrame({
                        "feature": selected_features,
                        "weight": [feature_weights[f] for f in selected_features],
                    }),
            ...
            with st.spinner("2段目: 加工 + 元画像特徴 → 重要特徴量 モデルを学習中 ..."):
                ...
                ax.barh(r2_df2["feature"], r2_df2["Test_R2"])
                ax.set_xlabel("Test R²")
                ax.set_title("2段目: 特徴量ごとの当てはまり（CV）")
                ax.grid(axis="x", linestyle="--", alpha=0.6)
                st.pyplot(fig)

                if use_grid2:
                    st.write("2段目ベストパラメータ:", best_params2)
                else:
                    st.write("2段目は GridSearch なし（デフォルトパラメータ）で学習しました。")

                X2_means = X2.mean()

            # ★ ここで止めるオプション ★
            if not do_inverse:
                st.info("ここで処理を終了します（逆推定・ベスト加工探索はまだ実行していません）。\n"
                        "この表・グラフをそのままパワポに貼れば、2段目モデルの精度が説明できます。")
                return
            ...
            with st.spinner("新しい画像の特徴量計算中..."):
                ...
                if missing_feats:
                    st.warning(
                        f"新画像から取得できなかった特徴量: {missing_feats} "
                        f"→ データセットの平均値で補完します。"
                    )
                ...
                st.subheader("加工前の予測値")
                st.write(f"{pupil_col} の予測値（加工前）: **{pupil_before:.3f}**")
                st.write(f"z（縮瞳に効く特徴の合成指標, 加工前）: **{z_before:.3f}**")
                ...
                st.caption(
                    f"各行の『寄与 w_i * z_i』を足し合わせた値が z ≒ **{z_before:.3f}** になります。"
                )

            # -------------------------
            # 18パターン × ランダムサーチ
            # -------------------------
            with st.spinner("18パターン × ランダムサーチで候補を生成中..."):
                allowed_patterns = generate_allowed_patterns()
                sim_records = []

                # orig 特徴量ベクトル（2段目の説明変数用）
                orig_cols = [
                    c for c in df_full.columns
                    if c.endswith("_orig") or c.endswith("_orig_area") or c.endswith("_orig_pupil")
                ]
                ...
                if new_image_file is not None:
                    # 2段目入力用の *_orig は fallback 行から借りる（暫定）
                    for c in orig_cols:
            ...
            # -------------------------
            # SSIM を用いた画質評価 + ベスト加工選択
            # -------------------------
            if new_image_for_display is None:
                st.warning(
                    "新しい画像がアップロードされていないため、SSIM による画質評価は行えません。"
                    "z 最大の候補をそのままベスト加工として採用します。"
                )
                best_row = sim_all.loc[sim_all["Score"].idxmax()].copy()
                best_ssim = np.nan
                cand_for_plot = sim_all.copy()
            else:
                with st.spinner("上位候補に対して SSIM を計算中..."):
                    # z の大きい順に上位 N 候補だけ SSIM を計算
                    cand_for_q = sim_all.sort_values("Score", ascending=False).head(
                        max_candidates_for_quality
                    ).copy()

                    ssim_list = []
                    J_list = []

                    img_ref_pil = new_image_for_display

                    best_feasible_row = None
                    best_feasible_ssim = None

                    for idx, row in cand_for_q.iterrows():
                        ops = [row["step1_op"], row["step2_op"], row["step3_op"]]
                        vals = [row["step1_val"], row["step2_val"], row["step3_val"]]
                        img_proc = apply_processing_sequence(img_ref_pil, ops, vals)

                        q_ssim = compute_ssim_pil(img_ref_pil, img_proc)
                        ssim_list.append(q_ssim)

                        # 合成指標 J も同時に計算
                        J_val = alpha_J * row["Score"] - beta_J * (1.0 - q_ssim)
                        J_list.append(J_val)

                        # 制約付きモード: SSIM が閾値以上の最初の候補を採用
                        if (
                            optimization_mode.startswith("制約付き")
                            and best_feasible_row is None
                            and q_ssim >= ssim_threshold
                        ):
                            best_feasible_row = row.copy()
                            best_feasible_ssim = q_ssim

                    cand_for_q["SSIM"] = ssim_list
                    cand_for_q["J"] = J_list
                    cand_for_plot = cand_for_q.copy()

                    if optimization_mode.startswith("制約付き"):
                        if best_feasible_row is not None:
                            best_row = best_feasible_row
                            best_ssim = best_feasible_ssim
                            st.success(
                                f"SSIM ≥ {ssim_threshold:.2f} を満たす候補の中で、"
                                f"z が最大の加工を選択しました（SSIM = {best_ssim:.3f}）。"
                            )
                        else:
                            best_row = cand_for_q.iloc[0].copy()
                            best_ssim = cand_for_q["SSIM"].iloc[0]
                            st.warning(
                                "指定した SSIM 閾値を満たす候補が上位 N 個の中にありませんでした。"
                                "とりあえず z が最大の候補を採用しています。"
                            )
                    else:
                        # J 最大の候補を採用
                        best_idx = cand_for_q["J"].idxmax()
                        best_row = cand_for_q.loc[best_idx].copy()
                        best_ssim = cand_for_q.loc[best_idx, "SSIM"]
                        st.success(
                            f"合成指標 J = α·z − β·(1−SSIM) が最大となる加工を選択しました（SSIM = {best_ssim:.3f}）。"
                        )

                # z vs pupil vs SSIM の散布図
                st.subheader("パターン別シミュレーション結果の散布図（z × pupil × SSIM）")
                fig_sc, ax_sc = plt.subplots(figsize=(8, 6))
                sc = ax_sc.scatter(
                    cand_for_plot["Score"],
                    cand_for_plot["Pupil"],
                    c=cand_for_plot["SSIM"],
                    alpha=0.6,
                )
                ax_sc.set_xlabel("Score (z)")
                ax_sc.set_ylabel("Predicted pupil")
                ax_sc.grid(True, linestyle="--", alpha=0.5)
                cbar = fig_sc.colorbar(sc)
                cbar.set_label("SSIM (Q)")
                st.pyplot(fig_sc)

                st.caption(
                    f"z の大きい順に上位 {len(cand_for_plot)} 個の候補について SSIM を計算し、"
                    "z と pupil のトレードオフを SSIM の色で可視化しています。"
                )

                st.subheader("SSIM を計算した候補の一覧（上位 z）")
                st.dataframe(
                    cand_for_plot[
                        ["pattern", "Score", "Pupil", "SSIM", "J",
            ...
            st.divider()
            st.subheader("👑 この画像に対するベスト加工案（画質制御込み）")
            ...
            df_feats = pd.DataFrame(
                {
                    ...
                    "ベスト加工後": [feat_after_vec[f] for f in selected_features],
                    "重みw": [feature_weights[f] for f in selected_features],
                }
            )

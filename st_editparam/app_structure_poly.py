...
#     # brightness: prefer bl (exclude bnl), fallback by keywords
...
# # =========================
# # Interaction term (ONLY c*s)
# # =========================
# def interaction_name(c_feat: str, s_feat: str) -> str:
#     return f"int__({c_feat})x({s_feat})"


# def add_cs_interaction_to_pair(Xtr_s: pd.DataFrame, Xte_s: pd.DataFrame, c_feat: str, s_feat: str):
...
# def add_cs_interaction_inplace(X_s: pd.DataFrame, c_feat: str, s_feat: str):
#     name = interaction_name(c_feat, s_feat)
#     X_s = X_s.copy()
#     X_s[name] = X_s[c_feat] * X_s[s_feat]
#     return X_s, name
...
#         int_name = None
#         if use_interaction:
#             Xtr_s, Xte_s, int_name = add_cs_interaction_to_pair(Xtr_s, Xte_s, c_feat, s_feat)
...
#     # in-sample: 全データでスケーリング→OLS
#     X_s = Xlib[list(feats)].copy()
#     sc = StandardScaler()
#     X_s = pd.DataFrame(sc.fit_transform(X_s), index=X_s.index, columns=X_s.columns)
...
#         X_s, int_name = add_cs_interaction_inplace(X_s, c_feat, s_feat)
...
# #  - contrast  coef < 0
# #  - |brightness| > |contrast|
# #  - at least 2 of (b,c,s) have p < alpha
# #    (optional) include interaction p in the sig-count when interaction is used
# # =========================
...
#         "B_terms_sigcount": np.nan,       # (b,c,s) だけ
#         "B_terms_sigcount_rule": np.nan,  # ルール側（交互作用含める場合は4項）
...
#     # in-sample coef/p for A（切片含む）
#     try:
#         _, tabA, _ = insample_fit_and_table(Xlib_A, y, [bestA_feat], robust_hc3, use_interaction=False)
...
# st.title("🌞🌙 昼/夜：回帰（A=輝度1項） vs（B=b+c+s（+ コントラスト×シャープネス候補））")

# # 入力（固定パスだけど、UIで変えられるようにしておく）
# with st.sidebar:
#     st.subheader("入力ファイル")
#     day_path_in = st.text_input("昼ファイルパス", value=DEFAULT_DAY_PATH)
#     night_path_in = st.text_input("夜ファイルパス", value=DEFAULT_NIGHT_PATH)

# day_path = _resolve_path(day_path_in)
# night_path = _resolve_path(night_path_in)

# st.caption(f"読み込み: day={day_path} / night={night_path}")

# if not os.path.exists(day_path) or not os.path.exists(night_path):
#     st.error("ファイルが見つかりません。サイドバーのパスを確認して。")
#     st.stop()

# df_day = load_any_path(day_path_in)
# df_night = load_any_path(night_path_in)
...
# with o3:
#     add_sqrt = st.checkbox("sqrt(符号付きsqrt) を候補に追加", value=False)
# with o4:
#     allow_interaction_candidate = st.checkbox("交互作用(コントラスト×シャープネス)も“探索候補”に入れる", value=False)
# with o5:
#     robust_hc3 = st.checkbox("p値をHC3ロバストで計算（昼の不安定対策）", value=True)

# st.caption("※Bモデルは基本は **b+c+sの3項固定**。交互作用ONの場合は **(b+c+s)** と **(b+c+s+c×s)** の両方を候補にしてR²で選びます。")
...
#     # 重要：ここで「4列しか出ない」事故が起きないように、show_cols を固定で全部並べる
#     show_cols = [
...
#         # 切片（const）
#         "A_const_coef", "A_const_p",
#         "B_const_coef", "B_const_p",

#         # A詳細
#         "A_best_feat", "A_coef", "A_p",
...
#     st.dataframe(
#         df_out[show_cols].sort_values(["dataset", "feature_group_name"]),
#         use_container_width=True
#     )

#     st.subheader("📌 R²だけ（比較用）")
#     slim_cols = ["dataset", "feature_group_name", "A_R2", "B_R2", "Delta(B-A)", "accepted", "B_use_interaction"]
#     for c in slim_cols:
#         if c not in df_out.columns:
#             df_out[c] = np.nan
#     st.dataframe(df_out[slim_cols].sort_values(["dataset", "feature_group_name"]), use_container_width=True)

#     csv = df_out.to_csv(index=False).encode("utf-8-sig")
#     st.download_button("CSVでダウンロード", data=csv, file_name="day_night_reg_results.csv", mime="text/csv")

#     with st.expander("説明"):
#         st.markdown(
#             "- 読み込みはサイドバーのパス（デフォルトは DEFAULT_*）。\n"
#             "- Bは (b,c,s) を必ず入れ、交互作用ONなら (b,c,s) と (b,c,s,c×s) を両方試して良い方を採用します。\n"
#             "- 採用条件は：\n"
#             "  1) R²_B > R²_A\n"
#             "  2) 輝度係数 < 0、コントラスト係数 < 0\n"
#             "  3) |輝度係数| > |コントラスト係数|\n"
#             "  4) p<α が2項以上（交互作用pを含めるかはチェックボックス）\n"
#             "- 切片(const)も A/B ともに係数・p値を出します。"
#         )

# app_day_night_reg_only.py
# streamlit run app_day_night_reg_only.py

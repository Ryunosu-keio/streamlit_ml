...
#
# ★変更要件（今回）:
#  - Stage2(Model2) の候補を RF, LightGBM, SV, Polynomial, NN のみにする
# ============================================================
...
def predict_stage1_from_x(m1, x: pd.Series, stage1_task: str) -> dict:
    # ★列名付き DataFrame で統一（LazyPredictやPipelineでも安全）
    X = x.to_frame().T
    if stage1_task == "reg":
...
# --- Stage2: candidate models (RF, LightGBM, SV, Polynomial, NN) ---
RF_PARAM_GRID_STAGE2 = {
    "n_estimators": [200, 400],
    "max_depth": [None, 10],
    "min_samples_leaf": [1, 3],
}
LGBM_PARAM_GRID_STAGE2 = {
...
SVR_PARAM_GRID_STAGE2 = {
    "C": [1.0, 10.0],
    "epsilon": [0.01, 0.1],
    "gamma": ["scale", "auto"],
}
POLY_PARAM_GRID_STAGE2 = {
    "degree": [2, 3],
    "alpha": [0.1, 1.0, 10.0],
}
NN_PARAM_GRID_STAGE2 = {
...
STAGE2_PARAM_GRIDS = {
    "RandomForest": RF_PARAM_GRID_STAGE2,
    "LightGBM": LGBM_PARAM_GRID_STAGE2,
    "SVR": SVR_PARAM_GRID_STAGE2,
    "Polynomial": POLY_PARAM_GRID_STAGE2,
    "NeuralNet": NN_PARAM_GRID_STAGE2,
}
...
def create_stage2_base(model_type: str, params: dict):
    """
    Stage2 base estimator (single-output). MultiOutputRegressorで包む前提。
    model_type: RandomForest / LightGBM / SVR / Polynomial / NeuralNet
    """
    params = params or {}

    if model_type == "RandomForest":
        base = {"n_estimators": 400, "random_state": 42, "n_jobs": -1}
        base.update(params)
        return RandomForestRegressor(**base)

    if model_type == "LightGBM":
        if not LGBM_AVAILABLE:
            raise RuntimeError("LightGBM is not available. pip install lightgbm")
        base = {
            "random_state": 42,
            "n_estimators": 600,
            "learning_rate": 0.05,
    ...
    if model_type == "SVR":
        base = {"kernel": "rbf", "C": 10.0, "epsilon": 0.05, "gamma": "scale"}
        base.update(params)
        return SVR(**base)
    ...
    if model_type == "Polynomial":
        ...
        # 注意: 入力次元が大きいと増えるので、degreeはUIで慎重に
        return Pipeline([
    ...
    raise ValueError(f"Unknown stage2 model_type: {model_type}")
...
def train_stage2_simple(X2, Y2, w, groups, model_type: str):
    splitter, is_group = _get_splitter(groups)
    splits = list(splitter.split(X2, Y2, groups)) if is_group else list(splitter.split(X2, Y2))
    prog = st.progress(0.0, text=f"Stage2 training ({model_type})...")
    ...
    for i, (tr_idx, te_idx) in enumerate(splits):
        ...
        prog.progress((i + 1) / len(splits), text=f"Stage2 training ({model_type})... ({i+1}/{len(splits)})")
...
def grid_search_stage2(X2, Y2, w, groups, model_type: str):
    param_grid = STAGE2_PARAM_GRIDS.get(model_type, {})
    total = int(np.prod([len(v) for v in param_grid.values()])) if param_grid else 1
    prog = st.progress(0.0, text=f"Stage2 GridSearch ({model_type})... (0/{total})")
    ...
    for params in iter_param_grid(param_grid):
        ...
        prog.progress(done / total, text=f"Stage2 GridSearch ({model_type})... ({done}/{total})")

    base_final = create_stage2_base(model_type, best_params or {})
    mo2 = MultiOutputRegressor(base_final)
    try:
        mo2.fit(X2, Y2, sample_weight=w)
    except TypeError:
...
def regression_block(
    ...
    default_sharp_kw=("sharp", "lap", "tenengrad", "acut", "hf", "highfreq"),
):
    st.subheader(title)
    if not SM_AVAILABLE:
...
def lazypredict_select_model(X: pd.DataFrame, y: pd.Series, task: str, groups: pd.Series | None):
    ...
    if task == "reg":
        ...
        models_df, _ = lazy.fit(X_tr, X_te, y_tr, y_te)
...
def main():
    ...
    st.sidebar.write(f"- lightgbm: {'OK' if LGBM_AVAILABLE else 'NG'}")
    if not SM_AVAILABLE:
    ...
    if not LGBM_AVAILABLE:
        st.sidebar.caption("LightGBM: pip install lightgbm")
    ...
    with tabs[2]:
        ...
        # === Stage2 model options (RF, LightGBM, SV, Polynomial, NN) ===
        st.markdown("### 🧬 Stage2 モデル（加工パラメータ + *_orig → 特徴量）")
        m2_options = ["RF", "LightGBM", "SV", "Polynomial", "NN"]
        if not LGBM_AVAILABLE and "LightGBM" in m2_options:
            # LightGBMがない環境では選択肢から外す
            m2_options = ["RF", "SV", "Polynomial", "NN"]
            st.info("LightGBM が未導入のため、Stage2 の選択肢から除外しました（pip install lightgbm）。")

        m2_label = st.radio("モデル2（候補）", m2_options, index=0, horizontal=True)
        ...
        if model2_type == "Polynomial":
            st.caption("Polynomial は入力次元が大きいと重くなります。まず degree=2 を推奨。")
        if model2_type == "SVR":
            st.caption("SVR は MultiOutput で出力数ぶん学習するので重くなりやすいです。")
        if model2_type == "NeuralNet":
            st.caption("NN は学習が不安定な場合があります（max_iter/alpha などを調整）。")
        ...
        if quality_metric_key == "SSIM":
            q_th = st.slider("SSIM(Luma) の閾値", 0.5, 1.0, 0.7, 0.01)
        else:
        ...
        st.subheader(f"Stage2 CV（特徴量の予測）: {trained.get('model2_type')}")
        ...
        if st.button("🔍 推奨実行（高速探索 → 画質評価 → A/B/C表示）"):
            ...
            if stage1_task == "reg":
                pupil_before = float(m1.predict(x_before.to_frame().T)[0])
                st.write(f"予測瞳孔: **{pupil_before:.3f}**")
                st.write(f"z スコア: **{z_before:.3f}**")
            else:
                p_shrink_before = float(m1.predict_proba(x_before.to_frame().T)[:, 1][0])
            ...
            with st.spinner("高速探索（Model2で特徴推定 → Stage1で目的推定）..."):
                ...
                for pi, pat in enumerate(allowed):
                    ...
                    Y_pred_df = pd.DataFrame(Y_pred_feats, columns=selected)
            ...
            if new_img_pil is not None:
                ...
                rows = []
                if stage1_task == "reg":
                    rows.append({"Image": "A", "Pred_pupil": pred_a["pupil"], "P(shrink)": np.nan, "meanLuma": mean_luma_255(img_a)})
                    rows.append({"Image": "B", "Pred_pupil": pred_b["pupil"], "P(shrink)": np.nan, "meanLuma": mean_luma_255(img_b)})
                    rows.append({"Image": "C", "Pred_pupil": pred_c["pupil"], "P(shrink)": np.nan, "meanLuma": mean_luma_255(img_c)})
                else:
                    rows.append({"Image": "A", "Pred_pupil": np.nan, "P(shrink)": pred_a["p_shrink"], "meanLuma": mean_luma_255(img_a)})
                    rows.append({"Image": "B", "Pred_pupil": np.nan, "P(shrink)": pred_b["p_shrink"], "meanLuma": mean_luma_255(img_b)})
                    rows.append({"Image": "C", "Pred_pupil": np.nan, "P(shrink)": pred_c["p_shrink"], "meanLuma": mean_luma_255(img_c)})

                st.dataframe(pd.DataFrame(rows), use_container_width=True)

                st.subheader("Basic stats (A)")
                st.dataframe(image_basic_stats(img_a), use_container_width=True)

            else:
                st.info("画像がないため A/B/C 表示はスキップしました。")

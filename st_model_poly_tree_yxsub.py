...
def permutation_signal_test(task_type, X, y, groups=None, scheme="loso", n_perm=30, seed=42):
    ...
    # 片側の雑p（実スコアより良いシャッフルの割合）
    p = float((np.sum(np.array(perm_scores) >= real) + 1) / (len(perm_scores) + 1)) if task_type == "回帰" else \
        float((np.sum(np.array(perm_scores) >= real) + 1) / (len(perm_scores) + 1))

    return {
        "real": real,
        "perm_mean": perm_mean,
        "perm_sd": perm_sd,
        "p_like": p,  # “小さいほど信号あり”の目安（厳密検定ではない）
    }

def estimate_y_reliability_split_half(df, y_col, unit_cols):
    """
    同一条件（unit_cols）内で複数試行がある前提で、split-half 相関の平均を出す。
    unit_cols 例: ["folder_name","file_name"]（被験者×画像）
    """
...
def estimate_var_component_reliability(df, y_col, subject_col="folder_name", unit_col="file_name"):
    """
    ざっくり二層（被験者間 / ユニット内）で分散を見て、
    “k回平均したときの信頼性”を予測する（Spearman-Brown的）。
    """
    ...
    tmp = df[[subject_col] + ([unit_col] if unit_col else []) + [y_col]].copy()
    tmp[y_col] = pd.to_numeric(tmp[y_col], errors="coerce")
    tmp = tmp.dropna(subset=[y_col])
    if len(tmp) == 0:
        return None
...
def decide_bottleneck(diag):
    ...
    if delta is not None:
        if diag["task_type"] == "回帰":
            if delta < 0.02:
                reasons.append("実データがシャッフルと同程度→x側の信号が弱い/特徴量設計が不足")
        else:
            if delta < 0.02:
                reasons.append("実F1がシャッフルと同程度→x側の信号が弱い/ラベル定義が粗い可能性")

    if within is not None and loso is not None:
        if (within - loso) > 0.10:
    ...
    # 3) within>>LOSO なら被験者
        ...
        primary = "混合（y/x/被験者）を小さく改善：まずは学習曲線でデータ不足か確認"
...
with st.expander("診断の考え方（あなたが書いてくれた方針をそのまま実装）", expanded=False):
    st.markdown("""
- **yをよくする**：目的変数のノイズが大きいと、どんなモデルでも頭打ちになります（信頼性が低いと上限R²も低い）
- **xをよくする**：シャッフル（ランダムラベル）と同程度なら、特徴量が“効いてない”可能性が高い
- **被験者をよくする**：within-subject は良いのに LOSO が悪いなら、個人差が支配的（正規化/被験者増が効く）
""")

run_diag = st.button("この設定でボトルネック診断を実行")

if run_diag:
    # ---- データ確保（あなたのCVロジックに合わせる）----
    if mode == 'ランダム（単一ファイルから分割/CV）':
        if df is None:
            st.error("ファイルをアップロードしてください。")
            st.stop()
    ...
    # tr.dataset で X,Y を作る（あなたの既存仕様に合わせる）
    ...
    if task_type == "分類" and bin_choice != "そのまま使う":
        ...
        valid_idx = y_cont.notna()
        X, y_cont = X.loc[valid_idx], y_cont.loc[valid_idx]
        if groups_subj is not None:
            groups_subj = groups_subj.loc[valid_idx]

        y_bins = pd.qcut(y_cont, q=n_bins, labels=False, duplicates="drop")
        Y_use = pd.Series(y_bins.astype(int), index=y_cont.index, name=target)

    # ---- LOIO用グループ列を推定（file_name優先→なければfigure）----
    loio_group = None
    ...
    # ---- 1) yの信頼性（推定）----
    y_rel = estimate_var_component_reliability(base_df.loc[X.index], target, subject_col=group_col, unit_col="file_name" if "file_name" in base_df.columns else None)
    y_split = estimate_y_reliability_split_half(base_df.loc[X.index], target, unit_cols=[c for c in [group_col, "file_name"] if c in base_df.columns])

    # ---- 2) xの信号（Permutation）----
    # ここは“本当に新規被験者へ一般化するか”を見たいので LOSO をデフォに
    x_sig = permutation_signal_test(task_type, X, Y_use, groups=groups_subj, scheme="loso", n_perm=20, seed=42)

    # ---- 3) 被験者差：within vs LOSO vs LOIO ----
    scores = {}
    ...
    if y_rel is None:
        st.info("y信頼性（分散成分）の推定ができませんでした（必要列が足りない可能性）。")
    else:
        st.write(pd.DataFrame([y_rel]))
        st.caption("rel_k1 が低いほど、1試行のyがノイジーで“頭打ち”になりやすい。k回平均で改善する見込みも表示。")

    if y_split is not None:
        st.write(pd.DataFrame([y_split]))
        st.caption("split-half で同一(被験者×画像)の再現性を直接チェック（可能な場合）。")
    ...
    st.caption("real が perm_mean と同程度なら『xに信号がない/特徴量が効いてない/ラベル設計が粗い』可能性が高い。")

    st.markdown("### ③ 被験者差・一般化（within / LOSO / LOIO）")
    st.write(pd.DataFrame([scores]))
    st.caption("within ≫ LOSO なら『被験者差が支配的』＝正規化・被験者増・スクリーニングが効きやすい。")
